...
# 명령어를 서브프로세스로 실행하고 실패 시(returncode != 0) 즉시 예외를 던진다 (fail-fast)
...
# CLI 진입점: --pdf로 특정 파일만, 미지정 시 raw_pdfs 내 전체 PDF를 순회 처리
def main() -> None:
